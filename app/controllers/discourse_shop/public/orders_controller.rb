...
class DiscourseShop::Public::OrdersController < DiscourseShop::Public::BaseController
  ...
  def create
    ...
    order.save!
    DiscourseShop::Log.log!("[shop] order created ##{order.id} by u#{current_user.id} ip=#{request.remote_ip}")
